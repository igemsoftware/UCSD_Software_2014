...
$window, Network, VisualStyles, Gist) {
	...
	var vs = {};
        ...
        console.log('Network rendering start...');
         ...
         /** EVIL CODE: do NOT uncomment. This KILLS the graph. :( Took me 3 hours to get this bug..need to understand how it works :@
...
[DEFAULT_VISUAL_STYLE_NAME].style).update();
	...
	function updateNetworkData(cy) {
        ...
        function setEventListeners() {
            ...
            $scope.cy.on('tap', 'node', function(e){
      var node = e.cyTarget; 
      var neighborhood = node.neighborhood().add(node);
      
      $scope.cy.elements().addClass('faded');
      neighborhood.removeClass('faded');
    });
    
    $scope.cy.on('tap', function(e){
      if( e.cyTarget === cy ){
        $scope.cy.elements().removeClass('faded');
      }
    });
